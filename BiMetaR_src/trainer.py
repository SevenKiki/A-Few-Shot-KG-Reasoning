...
class Trainer:
    ...
    def train(self):
        ...
        for e in range(self.epoch):
            ...
            if e % self.print_epoch == 0:
                ...
                with open(self.parameter['prefix'] + '_' + self.parameter['data_form'] + '_loss.csv', 'a+') as f:
                    csv_write = csv.writer(f)
                    data = [str(e), str(loss_num)]
            ...
            if e % self.eval_epoch == 0 and e != 0:
                ...
                valid_data, eval_rst = self.eval(istest=False, epoch=e)
                ...
                if valid_data[metric] > best_value:
                    ...
                    with open(self.parameter['prefix'] + '_' + self.parameter['data_form'] + '_rst.csv', 'w') as f:
                        csv_write = csv.writer(f)
                        csv_head = ["id","label"]
                        csv_write.writerow(csv_head)
                        csv_write.writerows(eval_rst)
                else:
    ...
    def eval(self, istest=False, epoch=None):
        ...
        while True:
            ...
            num_n = len(eval_task[3][0])
            _, p_score, n_score = self.do_one_step(eval_task, iseval=True, curr_rel=curr_rel)

            x = torch.cat([n_score, p_score], 1).squeeze()

            idx = self.rank_predict(data, x, ranks)
            if(idx < num_n):
                eval_rst.append([str(t), eval_task[3][0][idx][2]])
            else:
                eval_rst.append([str(t), eval_task[2][0][idx-num_n][2]])
    ...
    def test(self):
        ...
        while True:
            # sample all the eval tasks
            test_task, curr_rel = data_loader.next_one_on_test()
            # at the end of sample tasks, a symbol 'EOT' will return
            if test_task == 'EOT':
                break

            score = self.metaR.predict(test_task, curr_rel)
            _, idx = torch.sort(score[0], descending=True)
            ent = test_task[1][0][idx[0].item()][2]
            if t == 0:
                with open(self.parameter['prefix'] + '_' + self.parameter['data_form'] + '_rst.csv', 'w') as f:
                    csv_write = csv.writer(f)
                    csv_head = ["id","label"]
                ...
                with open(self.parameter['prefix'] + '_' + self.parameter['data_form'] + '_rst.csv', 'a+') as f:
                    csv_write = csv.writer(f)
                    data = [str(t), ent]
                    csv_write.writerow(data)
