...
class DataLoader(object):   # !!!!!
    def __init__(self, dataset, parameter, step='train'):
        ...
        if step == 'dev':
            self.eval_triples = []
            id = 0
            for key in dataset['test_query'].keys():
                for e1rel in dataset['test_query'][key]:
                    e1rel.append(dataset['test_groundtruth'][id])
                    id += 1
                    self.eval_triples.append(e1rel)
        ...
        if step == 'test':
            self.test_queries = []
            for key in dataset['test_query'].keys():
                self.test_queries.extend(dataset['test_query'][key])
            self.num_tris = len(self.test_queries)
            self.curr_tri_idx = 0
    ...
    def next_one_on_test(self):
        if self.curr_tri_idx == self.num_tris:
            return "EOT", "EOT"

        # get current triple
        query_triple = self.test_queries[self.curr_tri_idx]
        head_ent = query_triple[0]
        self.curr_tri_idx += 1
        curr_rel = query_triple[1]
        curr_cand = self.rel2candidates[curr_rel]   # 候选集
        # curr_cand = self.e1rel_e2[query_triple[0]+curr_rel]
        # print(curr_cand)
        query_triples = [[query_triple+[t] for t in curr_cand if t != head_ent]]
        support_triple = [self.tasks[curr_rel]]

        return [support_triple, query_triples], curr_rel
